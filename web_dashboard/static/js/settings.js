...
class SettingsManager {
    ...
    bindEvents() {
        ...
        document.getElementById('btn-migration-check').addEventListener('click', () => this.checkMigration());
        document.getElementById('btn-migration-execute').addEventListener('click', () => this.executeMigration());
        document.getElementById('btn-migration-history').addEventListener('click', () => this.showMigrationHistory());
    }
    ...
    async loadMigrationStatus() {
        try {
            ...
            if (response.ok) {
                const status = await response.json();
                this.updateMigrationDisplay(status);
            }
        } catch (error) {
            console.error('Migration status load error:', error);
            this.showStatus('マイグレーション状況の読み込みに失敗しました', 'error');
        }
    }
    ...
    updateMigrationDisplay(status) {
        ...
        if (status.migration_available) {
            if (status.recent_migrations && status.recent_migrations.length > 0) {
                const latest = status.recent_migrations[0];
                ...
                statusElement.innerHTML = `<span class="badge ${badgeClass}">前回: ${latestStatus}</span>`;
            } else {
                statusElement.innerHTML = '<span class="badge bg-info">利用可能</span>';
            }
        } else {
            statusElement.innerHTML = '<span class="badge bg-secondary">利用不可</span>';
        }
        
        // Backup directory
        document.getElementById('migration-backup-dir').textContent = 
            status.backup_directory || '-';
        
        // Last run info
        if (status.recent_migrations && status.recent_migrations.length > 0) {
            const latest = status.recent_migrations[0];
            const lastRun = new Date(latest.timestamp_start).toLocaleString('ja-JP');
            document.getElementById('migration-last-run').textContent = lastRun;
        } else {
            document.getElementById('migration-last-run').textContent = '未実行';
        }
    }
    
    async checkMigration() {
        try {
            const checkBtn = document.getElementById('btn-migration-check');
            const executeBtn = document.getElementById('btn-migration-execute');
            
            // Loading state
            checkBtn.disabled = true;
            checkBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> チェック中...';
            
            this.showStatus('移行可能な銘柄をチェックしています...', 'info');
            
            const response = await fetch('/api/migration/dry-run', {
                method: 'POST'
            });
            
            if (response.ok) {
                const result = await response.json();
                this.displayMigrationResults(result, true); // dry-run結果
                
                // Enable execute button if symbols found
                if (result.total_symbols > 0) {
                    executeBtn.disabled = false;
                    this.showStatus(
                        `${result.total_symbols}個の銘柄が移行可能です。実行ボタンで移行を開始できます。`,
                        'success'
                    );
                } else {
                    this.showStatus('移行可能な銘柄が見つかりませんでした。', 'info');
                }
            } else {
                const error = await response.json();
                this.showStatus(`チェックエラー: ${error.error}`, 'error');
            }
            
            // Reset button
            checkBtn.disabled = false;
            checkBtn.innerHTML = '<i class="fas fa-search"></i> 移行可能銘柄をチェック';
            
        } catch (error) {
            console.error('Migration check error:', error);
            this.showStatus('マイグレーションチェックに失敗しました', 'error');
    ...
    async executeMigration() {
        if (!confirm('マイグレーションを実行しますか？\n\n・既存設定のバックアップが作成されます\n・移行可能な銘柄が新システムに追加されます\n・この操作は元に戻せません')) {
            return;
        }
        ...
        try {
            ...
            this.showStatus('マイグレーションを実行しています...', 'info');
            ...
            if (response.ok) {
                const result = await response.json();
                this.displayMigrationResults(result, false); // actual結果
                
                if (result.successful_migrations > 0) {
                    this.showStatus(
                        `マイグレーション完了！${result.successful_migrations}個の銘柄を移行しました。`,
                        'success'
                    );
                    
                    // Reload symbols list to show migrated symbols
                    await this.loadSettings();
                } else {
                    this.showStatus('マイグレーションが完了しましたが、移行された銘柄はありませんでした。', 'warning');
                }
            } else {
                const error = await response.json();
                this.showStatus(`マイグレーションエラー: ${error.error}`, 'error');
            }
            
            // Reset buttons
            executeBtn.disabled = true; // Keep disabled after execution
            checkBtn.disabled = false;
            executeBtn.innerHTML = '<i class="fas fa-play"></i> マイグレーション実行';
            ...
            this.showStatus('マイグレーションの実行に失敗しました', 'error');
    ...
    displayMigrationResults(result, isDryRun) {
        ...
        // Summary
        const summaryHtml = `
            <div class="migration-summary mb-3 p-3 border rounded">
                <div class="row">
                    <div class="col-md-3">
                        <strong>総銘柄数:</strong> ${result.total_symbols}
                    </div>
                    <div class="col-md-3">
                        <strong>成功:</strong> <span class="text-success">${result.successful_migrations}</span>
                    </div>
                    <div class="col-md-3">
                        <strong>失敗:</strong> <span class="text-danger">${result.failed_migrations}</span>
                    </div>
                    <div class="col-md-3">
                        <strong>警告:</strong> <span class="text-warning">${result.warnings_count}</span>
                    </div>
                </div>
                ${isDryRun ? '<div class="mt-2"><span class="badge bg-info">プレビュー結果</span></div>' : 
                    `<div class="mt-2"><span class="badge bg-success">実行完了</span> 実行ID: ${result.execution_id}</div>`}
            </div>
        `;
        
        // Details
        const detailsHtml = result.results.map(symbolResult => {
            const statusClass = {
                'success': 'success',
                'partial': 'warning', 
                'failed': 'danger'
            }[symbolResult.status] || 'secondary';
            
            const statusIcon = {
                'success': 'fa-check',
                'partial': 'fa-exclamation-triangle',
                'failed': 'fa-times'
            }[symbolResult.status] || 'fa-question';
            
            return `
                <div class="symbol-result mb-2 p-2 border rounded">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <strong>${symbolResult.symbol}</strong>
                            <span class="badge bg-${statusClass} ms-2">
                                <i class="fas ${statusIcon}"></i> ${symbolResult.status}
                            </span>
                            ${symbolResult.validation_valid ? 
                                '<span class="badge bg-success ms-1">有効</span>' : 
                                '<span class="badge bg-warning ms-1">要注意</span>'}
                        </div>
                        <div class="text-muted small">
                            ${symbolResult.migrated_to_new_system ? '新システムに追加済み' : '未追加'}
                        </div>
                    </div>
                    ${symbolResult.warnings && symbolResult.warnings.length > 0 ? `
                        <div class="warnings mt-1">
                            ${symbolResult.warnings.map(warning => 
                                `<small class="text-warning"><i class="fas fa-exclamation-triangle"></i> ${warning}</small>`
                            ).join('<br>')}
                        </div>
                    ` : ''}
                    ${symbolResult.error_message ? `
                        <div class="error mt-1">
                            <small class="text-danger"><i class="fas fa-times"></i> ${symbolResult.error_message}</small>
                        </div>
                    ` : ''}
                </div>
            `;
        ...
        resultsContent.innerHTML = summaryHtml + detailsHtml;
    }
    ...
    async showMigrationHistory() {
        try {
            ...
            if (response.ok) {
                ...
                if (status.recent_migrations && status.recent_migrations.length > 0) {
                    let historyHtml = '<div class="migration-history">';
                    
                    status.recent_migrations.forEach(migration => {
                        const date = new Date(migration.timestamp_start).toLocaleString('ja-JP');
                        const statusClass = {
                            'SUCCESS': 'success',
                            'PARTIAL_SUCCESS': 'warning',
                            'FAILED': 'danger',
                            'RUNNING': 'primary'
                        }[migration.status] || 'secondary';
                        
                        historyHtml += `
                            <div class="history-item mb-2 p-2 border rounded">
                                <div class="d-flex justify-content-between">
                                    <div>
                                        <span class="badge bg-${statusClass}">${migration.status}</span>
                                        <strong class="ms-2">実行ID: ${migration.execution_id}</strong>
                                    </div>
                                    <small class="text-muted">${date}</small>
                                </div>
                                ${migration.metadata ? `
                                    <div class="mt-1">
                                        <small class="text-muted">タイプ: ${migration.metadata.migration_type || 'Unknown'}</small>
                                    </div>
                                ` : ''}
                            </div>
                        `;
                    ...
                    document.getElementById('migration-results-content').innerHTML = `
                        <h6><i class="fas fa-history"></i> マイグレーション履歴</h6>
                        ${historyHtml}
                    `;
                } else {
                    this.showStatus('マイグレーション履歴がありません', 'info');
                }
            }
        } catch (error) {
            console.error('Migration history error:', error);
            this.showStatus('マイグレーション履歴の取得に失敗しました', 'error');
        }
    }
}
